      ...
      status_right_(nullptr)
{
  ...
  connect(ui->comboBox_classes, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
          &MainWindow::OnClassSelected);
...
void MainWindow::ManageClasses()
{
  ...
  connect(btnRemove, &QPushButton::clicked,
          [&]()
          {
            ...
            auto reply =
                QMessageBox::question(&dialog, "Remove Class", "Remove class '" + pc->name + "'?",
...
void MainWindow::LoadImageAtIndex(int index)
{
  ...
  if (QFile::exists(labelPath))
  {
    ...
    statusBar()->showMessage(
        QString("Loaded image %1/%2 with annotations").arg(index + 1).arg(image_list_.size()),
        3000);
  }
  else
  {
    ...
    statusBar()->showMessage(
        QString("Loaded image %1/%2 (no annotations)").arg(index + 1).arg(image_list_.size()),
        3000);
  }
...
void MainWindow::CreateNewProject()
{
  ...
  QString projectName = QInputDialog::getText(this, "New Project",
                                              "Project name:", QLineEdit::Normal, "MyDataset", &ok);
  ...
  const auto home = qgetenv("HOME");
  QString projectDir = QFileDialog::getExistingDirectory(this, "Select Project Location", home);
...
void MainWindow::ExecutePluginCommand(const QString& command, const QStringList& args)
{
  ...
  {
    QMessageBox::critical(this, "Plugin Error", "Failed to start plugin command:\n" + command);
    return;
  }
...
void MainWindow::ParseDetectionResults(const QString& json_output)
{
  ...
  {
    QMessageBox::warning(this, "Parse Error", "Plugin did not return valid JSON output.");
    return;
  }
  ...
  {
    QMessageBox::warning(this, "Parse Error", "Plugin output missing 'detections' array.");
    return;
  }
...
void MainWindow::ConfigurePlugin()
{
  ...
  connect(browseBtn, &QPushButton::clicked,
          ...
          {
            QString file =
                QFileDialog::getOpenFileName(&dialog, "Select Plugin Script", project_directory_,
  ...
  connect(addSettingBtn, &QPushButton::clicked,
          ...
          {
            bool ok;
            QString key = QInputDialog::getText(&dialog, "Add Setting",
                                                "Setting name:", QLineEdit::Normal, "", &ok);
            if (ok && !key.isEmpty())
            {
...
void MainWindow::RunBatchDetect()
{
  ...
  QMessageBox::StandardButton reply =
      QMessageBox::question(this, "Batch Detection", message, QMessageBox::Yes | QMessageBox::No);
...
QString MainWindow::GetProjectStatistics() const
{
  ...
  {
    html +=
        QString("<tr><td>⚠️</td><td><font color='blue'>%1 image%2 pending review</font></td></tr>")
            .arg(pending_review)
