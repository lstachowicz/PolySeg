...
void PolygonCanvas::LoadYoloAnnotations(const QString& filepath, const QVector<QColor>& class_colors)
{
    QFile file(filepath);
    ...
    std::cout << "Loaded " << polygons_.size() << " polygons from: " 
...
void PolygonCanvas::AddPolygonFromPlugin(const QVector<QPoint>& points, int class_id, const QColor& color)
{
    if (points.size() < 3)
    {
    ...
    std::cout << "Added plugin polygon with " << points.size() << " points (class_id=" 
              << class_id << ")" << std::endl;
}
...
void PolygonCanvas::SelectPolygon(const QPoint& pos)
{
    ...
    for (int i = polygons_.size() - 1; i >= 0; --i)
    {
        ...
        if (inside)
        {
            ...
            update();
            std::cout << "Selected polygon " << i << " (class_id=" 
                      << polygons_[i].class_id << ")" << std::endl;
            return;
        }
    }
    ...
    if (ctrl_pressed && selected_polygon_index_ >= 0 && selected_polygon_index_ < polygons_.size())
    {
        ...
        if (polygon.points.size() > 1)
        {
            ...
            for (int i = 0; i < polygon.points.size(); ++i)
            {
                ...
                float distance = DistanceFromPointToSegment(position, polygon.points[i], polygon.points[next_i]);
            ...
            if (insert_index != -1 && min_distance < 10.0f)
            {
                ...
                std::cout << "Inserted point at index: " << insert_index << " in selected polygon" << std::endl;
                return;
            }
        }
    }
