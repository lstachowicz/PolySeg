...
void ProjectConfig::AddClass(const QString& name, const QColor& color, int index)
{
  ...
  std::cout << "Added class: " << name.toStdString() 
            << " (id=" << pc.id << ", index=" << pc.index << ", color=" << color.name().toStdString() << ")" << std::endl;
}
...
void ProjectConfig::FromJson(const QJsonObject& json)
{
  ...
  std::cout << "Loaded project: " << project_name_.toStdString() 
            << " with " << classes_.size() << " classes" << std::endl;
}
...
void ProjectConfig::UpdateImageSplits(const QStringList& all_images)
{
  ...
  std::cout << "Updated splits: Train=" << GetTrainCount() 
            << " Val=" << GetValCount() 
